...
def train_model(model, train_loader, valid_loader, device, hp, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            ...
            output = model(batch_x).squeeze(dim=-1)  # only squeeze channel/feature dimension
...
def objective(trial, train_loader, valid_loader, device, input_len, epochs, search_space):
    hp = {}
    ...
    # per-layer params
    for i in range(hp['num_layers']):
        for param in ['units', 'kernel_size', 'activation', 'pool_size', 'dropout', 'dilation']:
            config = search_space[param]
            param_key = f'{param}_{i}'
            if config['type'] == 'int':
                hp[param_key] = trial.suggest_int(param_key, config['low'], config['high'])
            elif config['type'] == 'float':
                hp[param_key] = trial.suggest_float(param_key, config['low'], config['high'], log=config.get('log', False))
            elif config['type'] == 'categorical':
                hp[param_key] = trial.suggest_categorical(param_key, config['choices'])

    model = DynamicCNN(hp, input_len=input_len)
    model = train_model(model, train_loader, valid_loader, device, hp, epochs=epochs)
    metrics = evaluate_model(model, valid_loader, device)

    return metrics["average_precision"]


def run_optuna_pipeline(train_loader, valid_loader, device, input_len, epochs, n_trials, save_path, search_space):
    study = optuna.create_study(direction="maximize")
    study.optimize(lambda trial: objective(trial, train_loader, valid_loader, device, input_len, epochs, search_space),
                   n_trials=n_trials)

    best_params = study.best_trial.params
    best_model = DynamicCNN(best_params, input_len=input_len)
    best_model = train_model(best_model, train_loader, valid_loader, device, best_params, epochs)
...
def predict_onehot_sequence(model, sequence, device='cuda', seq_len=101):
    """Predict label + prob from DNA sequence for trained One-Hot CNN."""
    ...
    encoded = one_hot_encode_sequences(pd.Series([sequence]))[0]  # return [seq_len, 4]
    encoded = encoded.transpose(1, 0)    
    input_tensor = torch.tensor(encoded).unsqueeze(0).to(device)  # shape [1,4,seq_len]

    model.eval()
    with torch.no_grad():
        output = model(input_tensor)  # shape [1,1] or scalar
        prob = output.item()
